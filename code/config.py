...
class Config:
    ...
    # Model configuration unsupported sets
    ...
    def _load_keyvault_secrets(cls):
        ...
        by_secret = {}
        for attr_name, secret_ref in getattr(cls, "KEY_VAULT_SECRET_MAP", []):
            if "." in secret_ref:
                ...
                if has_json_key:
                    ...
                    for attr_name, json_key in refs:
                        if json_key is not None:
                            ...
                            if attr_name in kv_secrets:
                                continue
                            if val is not None and val != "":
                                kv_secrets[attr_name] = str(val)
                            else:
                                logging.debug(f"Key Vault: key '{json_key}' not found in secret '{secret_name}' (field {attr_name})")
                else:
                    for attr_name, json_key in refs:
                        if json_key is None and raw_value:
                            kv_secrets[attr_name] = raw_value
                            break
            except Exception as exc:
    ...
    def _validate_api_keys(cls):
        ...
        if provider == "openai":
            ...
            if not getattr(cls, "AZURE_OPENAI_ENDPOINT", ""):
                raise ValueError("AZURE_OPENAI_ENDPOINT not found for Azure provider")
...
def _initialize_config():
    ...
    CONFIG_VARIABLES = [
        ...
        # Tool Integration (none for this agent)
    ]
    ...
    for var_name in CONFIG_VARIABLES:
        ...
        if value and var_name == "LLM_TEMPERATURE":
            ...
            # Parse LLM_MODELS as JSON list if present
